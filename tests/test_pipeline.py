...
class TestDataLoader:
    ...
    def test_validate_dataset(self):
        ...
        df = pd.DataFrame(
            {"feature1": [1, 2, 3], "feature2": ["a", "b", "c"], "Churn": ["Yes", "No", "Yes"]}
        )
...
class TestDataPreprocessor:
    ...
    def test_handle_missing_values_drop(self):
        ...
        df = pd.DataFrame({"A": [1, 2, np.nan, 4, 5], "B": ["a", "b", "c", "d", "e"]})
    ...
    def test_encode_categorical(self):
        ...
        df = pd.DataFrame({"cat1": ["A", "B", "A", "C"], "cat2": ["X", "Y", "X", "Y"]})
    ...
    def test_split_data(self):
        ...
        df = pd.DataFrame(
            {"feature1": range(100), "feature2": range(100, 200), "target": [0] * 50 + [1] * 50}
        )

        preprocessor = DataPreprocessor()
        splits = preprocessor.split_data(df, target_column="target", test_size=0.2, random_state=42)
...
class TestChurnVisualizer:
    ...
    def test_plot_creation(self):
        ...
        df = pd.DataFrame({"feature1": [1, 2, 3, 4, 5], "Churn": ["Yes", "No", "Yes", "No", "Yes"]})
